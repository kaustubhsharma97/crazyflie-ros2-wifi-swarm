...
class CirclePathNode(Node):
    ...
    def _tick(self):
        ...
        # ── Phase 2: fly to absolute circle centre ──
            ...
            if self._pending is None:
                dist = math.sqrt((CIRCLE_CENTER_X - self.spawn_x)**2 +
                                 (CIRCLE_CENTER_Y - self.spawn_y)**2)
                travel = max(3.0, dist / 0.5)   # IDENTICAL to Raghav
                self._travel = travel
                self._pending = self._send_goto(
                    CIRCLE_CENTER_X, CIRCLE_CENTER_Y, FLY_Z, travel)
                self._wait_until = self._now() + int((travel + 0.5) * 1e9)
                self.get_logger().info(
                    f"Phase 2: To centre (dist={dist:.2f}m, t={travel:.1f}s)")
            elif self._pending.done() and self._past():
                self._pending = None
                self.theta = 0.0
                self.step_i = 0
        ...
        # known stable hover at the circle centre, pauses, THEN descends.
            ...
            if self._pending is None:
                self._pending = self._send_goto(
                    CIRCLE_CENTER_X, CIRCLE_CENTER_Y, FLY_Z, 2.0)
                self._wait_until = self._now() + int(3.0 * 1e9)  # travel + settle
