#!/usr/bin/env python3
"""
circle_path_node_v5.py - ROS2 Humble / rclpy - SINGLE-DRONE CIRCLE (cf231)
===================================================================
...
Run:  python3 circle_path_node_v5.py
"""
import os, csv, math
import numpy as np
import rclpy
...
# ── Circle parameters ─────────────────────────────────────────────────────────
RADIUS          = 1.2      # m  (bigger = looks round despite ~30cm LPS noise)
OMEGA           = 0.4      # rad/s (slow = drone reaches each waypoint, hugs the ring)
CIRCLE_DURATION = 22.0     # s (one full loop takes 2*pi/OMEGA ~= 12.6s; 20s > that)
...
# ── Outputs ───────────────────────────────────────────────────────────────────
CSV_PATH = os.path.expanduser("~/circle_v5_log.csv")
PNG_XY   = os.path.expanduser("~/circle_v5_xy_topdown.png")
PNG_3D   = os.path.expanduser("~/circle_v5_3d.png")
PNG_ERR  = os.path.expanduser("~/circle_v5_error.png")
PNG_Z    = os.path.expanduser("~/circle_v5_z.png")
...
class CirclePathNodeV5(Node):

    def __init__(self):
        super().__init__("circle_path_node_v5")
        ...
        self.get_logger().info(f"circle_path_node_v5 started (r={RADIUS}, z={FLY_Z}). Non-blocking.")
    ...
    def _finalize(self):
        ...
        self.get_logger().info("circle_path_node_v5 complete.")
