#!/usr/bin/env python3
"""
leader_follower_circle.py — Option A leader-follower, CIRCLE trajectory
(pure rclpy + crazyflie_interfaces, no cflib)

One process, two nodes on a single executor:
  * LeaderNode (cf231) flies a circle with RIM-ENTRY start (center placed one
    radius behind the hover point so the trajectory begins exactly where the
    drone already is). Tuned defaults kept: radius 0.8 m, omega 0.5 rad/s.
    Every commanded setpoint is broadcast on /leader/setpoint (PoseStamped),
    mission phase on /leader/phase (String).
...
Run (one terminal):
    python3 leader_follower_circle.py
    python3 leader_follower_circle.py --ros-args -p radius:=0.6 -p laps:=3

Failsafe demo (leader mutes its broadcast mid-trajectory, keeps flying and
lands itself; follower must failsafe-land):
    python3 leader_follower_circle.py --ros-args -p simulate_leader_loss_after:=15.0

Optional split across two terminals:
    python3 leader_follower_circle.py --role follower
    python3 leader_follower_circle.py --role leader
"""
...
class LeaderNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('radius', 0.8)            # circle radius [m] (tuned)
        self.declare_parameter('omega', 0.5)             # angular speed [rad/s] (tuned)
        ...
        self.total_time = 0.0            # trajectory duration [s]
        self.land_z0 = 0.0
        ...
        self.get_logger().info(
            f'leader up: ns={self.ns} traj=circle R={self.radius} '
            f'omega={self.omega} laps={self.laps} height={self.height:.2f} m'
...
def main():
    parser = argparse.ArgumentParser(
        description='Option A leader-follower (circle)')
