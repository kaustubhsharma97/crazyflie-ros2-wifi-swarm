#!/usr/bin/env python3
"""
leader_follower_square.py — Option A leader-follower, SQUARE trajectory
(pure rclpy + crazyflie_interfaces, no cflib)

One process, two nodes on a single executor:
  * LeaderNode (cf231) flies a square of side `side` at constant edge speed
...
Run (one terminal):
    python3 leader_follower_square.py
    python3 leader_follower_square.py --ros-args -p side:=1.2 -p laps:=3

Failsafe demo (leader mutes its broadcast mid-trajectory, keeps flying and
lands itself; follower must failsafe-land):
    python3 leader_follower_square.py --ros-args -p simulate_leader_loss_after:=15.0

Optional split across two terminals:
    python3 leader_follower_square.py --role follower
    python3 leader_follower_square.py --role leader
"""
...
class LeaderNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('side', 1.0)              # square side length [m]
        ...
        self.get_logger().info(
            f'leader up: ns={self.ns} traj=square side={self.side} '
    ...
    def build_trajectory(self):
        ...
        L = self.side
        # vertices of the closed square loop, starting at the hover point
        ...
        self.get_logger().info(
            f'square: side={L} perimeter={self.perimeter:.2f} m '
...
def main():
    parser = argparse.ArgumentParser(
        description='Option A leader-follower (square)')
